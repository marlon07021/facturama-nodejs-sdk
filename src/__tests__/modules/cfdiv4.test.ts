...
describe('CFDIv4 module API', () => {
  ...
  test('get CFDI by id', async () => {
    ...
    expect(axios.get).toHaveBeenCalledWith(`${sdk.ApiUrl}api-lite/cfdis/${id}`, {
  ...
  test('list CFDIs', async () => {
    ...
    expect(axios.get).toHaveBeenCalledWith(`${sdk.ApiUrl}api-lite/cfdis`, {
      headers: { Authorization: 'Basic cHJ1ZWJhczpwcnVlYmFzMjAxMQ==', 'Content-Type': 'application/json' },
      signal: undefined,
    });
  });
