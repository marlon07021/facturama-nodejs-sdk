import FacturamaSDK from "../../facturama";
import axios from "axios";
import CSD from "../../modules/CSD/CSD";

jest.mock('axios');

const sampleCSD = {
    "Rfc": "XIA190128J61",
    "Certificate": "",
    "PrivateKey": "",
    "PrivateKeyPassword": "12345678a",
    "UploadDate": "2021-06-27T19:21:57.283"
}

describe('CSD module API', () => {
    const sdk = new FacturamaSDK(false);
    sdk.login('pruebas', 'pruebas2011');

    test('return a single result for RFC', async () => {

        const rfc = sampleCSD.Rfc;

        (axios.get as jest.Mock).mockResolvedValue(
            {data: sampleCSD}
        );

        const {data} = await sdk.CSD.Get(rfc);
        expect(data).toBe(sampleCSD);
        expect(axios.get).toHaveBeenCalledWith(`${sdk.ApiUrl}api-lite/csds/${rfc}`, {
            "headers": {
                "Authorization": "Basic cHJ1ZWJhczpwcnVlYmFzMjAxMQ==",
                "Content-Type": "application/json"
            }, "signal": undefined
        });
    });

    test('return a list', async () => {
        (axios.get as jest.Mock).mockResolvedValue({
            data: [
                sampleCSD
            ]
        });

        const {data} = await sdk.CSD.List();
        expect(data).toHaveLength(1);
        expect(axios.get).toHaveBeenCalledWith(`${sdk.ApiUrl}api-lite/csds`, {
            "headers": {
                "Authorization": "Basic cHJ1ZWJhczpwcnVlYmFzMjAxMQ==",
                "Content-Type": "application/json"
            }, "signal": undefined
        })
    });

    test('create a new CSD', async () => {

        const obj = new CSD(sampleCSD as any);

        // (axios.post as jest.Mock).mockResolvedValue(
        //     {data: sampleCSD}
        // );

        await sdk.CSD.Create(obj);
        expect(axios.post).toHaveBeenCalledWith(`${sdk.ApiUrl}api-lite/csds`, {...obj.json()},{

            "headers": {
                "Authorization": "Basic cHJ1ZWJhczpwcnVlYmFzMjAxMQ==",
                "Content-Type": "application/json"
            },
            "signal": undefined,

        });
    });

    test('update CSD', async () => {

        const rfc = sampleCSD.Rfc;
        const obj = new CSD({...(sampleCSD as any), PrivateKeyPassword: '02345678b'});

        //(axios.put as jest.Mock).mockResolvedValue();

        await sdk.CSD.Update(rfc, obj);

        expect(axios.put).toHaveBeenCalledWith(`${sdk.ApiUrl}api-lite/csds/${rfc}`, {...obj.json()},{

            "headers": {
                "Authorization": "Basic cHJ1ZWJhczpwcnVlYmFzMjAxMQ==",
                "Content-Type": "application/json"
            },
            "signal": undefined,

        });
    });

    test('remove CSD', async () => {

        const rfc = sampleCSD.Rfc;
        const obj = new CSD({...(sampleCSD as any), PrivateKeyPassword: '02345678b'});

        //(axios.put as jest.Mock).mockResolvedValue();

        await sdk.CSD.Remove(rfc);

        expect(axios.delete).toHaveBeenCalledWith(`${sdk.ApiUrl}api-lite/csds/${rfc}`,{
            "data": {},
            "headers": {
                "Authorization": "Basic cHJ1ZWJhczpwcnVlYmFzMjAxMQ==",
                "Content-Type": "application/json"
            },
            "signal": undefined,

        });
    });


});
